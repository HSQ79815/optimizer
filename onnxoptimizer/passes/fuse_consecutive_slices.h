...
struct FuseConsecutiveSlices final : public PredicateBasedPass {
  ...
  bool patternMatchPredicate(Node *node) override {
    ...
    return CheckKind(node, kSlice) && CheckKind(node->input(0), kSlice) &&
           node->inputs().size() == 5 &&
           node->input(0)->node()->inputs().size() == 5 &&
           FetchIntsOfTensor(node->input(3), slice1_axes) &&
           FetchIntsOfTensor(node->input(0)->node()->input(3), slice2_axes) &&
